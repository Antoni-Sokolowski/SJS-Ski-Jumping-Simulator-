...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # === POCZĄTEK ZMIANY: Sortowanie list ===
        self.all_jumpers.sort(key=lambda j: str(j))
        self.all_hills.sort(key=lambda h: str(h))
        # === KONIEC ZMIANY ===
    ...
    def run_simulation(self):
        ...
        try:
            ...
            self.positions = [(0, 0)]
            downward_angle_rad = -self.selected_hill.alpha_rad

            initial_total_velocity = inrun_simulation(self.selected_hill, self.selected_jumper, gate_number=gate)
            initial_velocity_x = initial_total_velocity * math.cos(downward_angle_rad)
            initial_velocity_y = initial_total_velocity * math.sin(downward_angle_rad)
            velocity_takeoff = (self.selected_jumper.jump_force * 0.1) / self.selected_jumper.mass
            velocity_takeoff_x = velocity_takeoff * math.sin(-downward_angle_rad)
            velocity_takeoff_y = velocity_takeoff * math.cos(-downward_angle_rad)
            ...
            ax.plot(self.x_landing, self.y_landing, color='#00aaff', linewidth=3)

            max_y_inrun = y_inrun[0] if len(y_inrun) > 0 else 0
            ax.set_xlim(-inrun_length_to_show - 5, max_hill_length + 10)
            ax.set_ylim(min(self.y_landing) - 5, max(max_height * 1.5, max_y_inrun) + 5)
            ...
            self.ani = animation.FuncAnimation(self.figure, update, init_func=init,
                                               frames=len(self.positions), interval=5,
